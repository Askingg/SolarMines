...
public enum CharLength {
    ...
    private CharLength(final String name, final int ordinal, final char character, final int length) {
    ...
    public int getBoldLength() {
        if (this == CharLength.SPACE) {
            return this.getLength();
        }
        return this.length + 1;
    }
    
    public static CharLength getDefaultFontInfo(final char c) {
    	CharLength[] values;
        for (int length = (values = values()).length, i = 0; i < length; ++i) {
            final CharLength dFI = values[i];
            if (dFI.getCharacter() == c) {
                return dFI;
            }
        }
        return CharLength.DEFAULT;
    }
